...
def define_sign(number):
    ...
    for i in set1,set2,set3,set4,set5,set6,set7,set8:
        for j in list(i):
            plyr_1=all(elm in player_1 for elm in j)
            plyr_2=all(elm in player_2 for elm in j)
            if plyr_1==True:
                print("Player 1 is the winner")
                showinfo("Game Result", "Player 1 has won the game!")
                askyesno("New Game", "Would you like to play again?")
                break
            elif plyr_2==True:
                print("Player 2 is the winner")
                showinfo("Game Result", "Player 2 has won the game!")
                askyesno("New Game", "Would you like to play again?")
                break
            else:
                pass
...
btnNewGame= Button(root, text='Play Again', font=('Times 15 bold'), height=3, width=20, command=NewGame, justify=CENTER)
btnNewGame.grid(row=4)
root.mainloop()
